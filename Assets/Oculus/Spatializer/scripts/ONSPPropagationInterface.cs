...
internal class ONSPPropagation
{
    ...
    public class UnityNativeInterface : PropagationInterface
    {
        ...
        public int SetPropagationQuality(float quality) => ovrAudio_SetPropagationQuality(context, quality);

        public int SetPropagationThreadAffinity(ulong cpuMask) => ovrAudio_SetPropagationThreadAffinity(context, cpuMask);

        public int CreateAudioGeometry(out IntPtr geometry) => ovrAudio_CreateAudioGeometry(context, out geometry);

        public int DestroyAudioGeometry(IntPtr geometry) => ovrAudio_DestroyAudioGeometry(geometry);

        public int AudioGeometryUploadMeshArrays(IntPtr      geometry,
                                                 float[]     vertices, int vertexCount,
                                                 int[]       indices,  int indexCount,
                                                 MeshGroup[] groups,   int groupCount) =>
                ovrAudio_AudioGeometryUploadMeshArrays(geometry,
        ...
        public int AudioGeometrySetTransform(IntPtr geometry, float[] matrix4x4) => ovrAudio_AudioGeometrySetTransform(geometry, matrix4x4);

        public int AudioGeometryGetTransform(IntPtr geometry, out float[] matrix4x4) => ovrAudio_AudioGeometryGetTransform(geometry, out matrix4x4);

        public int AudioGeometryWriteMeshFile(IntPtr geometry, string filePath) => ovrAudio_AudioGeometryWriteMeshFile(geometry, filePath);

        public int AudioGeometryReadMeshFile(IntPtr geometry, string filePath) => ovrAudio_AudioGeometryReadMeshFile(geometry, filePath);

        public int AudioGeometryWriteMeshFileObj(IntPtr geometry, string filePath) => ovrAudio_AudioGeometryWriteMeshFileObj(geometry, filePath);

        public int CreateAudioMaterial(out IntPtr material) => ovrAudio_CreateAudioMaterial(context, out material);

        public int DestroyAudioMaterial(IntPtr material) => ovrAudio_DestroyAudioMaterial(material);

        public int AudioMaterialSetFrequency(IntPtr material, MaterialProperty property, float frequency, float value) => ovrAudio_AudioMaterialSetFrequency(material, property, frequency, value);

        public int AudioMaterialGetFrequency(IntPtr material, MaterialProperty property, float frequency, out float value) => ovrAudio_AudioMaterialGetFrequency(material, property, frequency, out value);

        public int AudioMaterialReset(IntPtr material, MaterialProperty property) => ovrAudio_AudioMaterialReset(material, property);
    ...
    public class WwisePluginInterface : PropagationInterface
    {
        ...
        private IntPtr context
        {
            get
            {
                if (context_ == IntPtr.Zero) { ovrAudio_GetPluginContext(out context_, ClientType.OVRA_CLIENT_TYPE_WWISE_UNKNOWN); }
