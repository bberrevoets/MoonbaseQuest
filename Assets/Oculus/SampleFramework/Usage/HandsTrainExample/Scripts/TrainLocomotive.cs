...
namespace OculusSampleFramework
{
    public class TrainLocomotive : TrainCarBase
    {
        ...
        {
            var endSpeed = startTrain ? _initialSpeed : 0.0f;
            ...
            var endTime    = Time.time + timePeriodForSpeedChange;
            var startSpeed = _currentSpeed;
            while (Time.time < endTime)
            {
                var t = (Time.time - startTime) / timePeriodForSpeedChange;
        ...
        private float PlayEngineSound(EngineSoundState engineSoundState)
        {
            ...
            {
                var audioClips = engineSoundState == EngineSoundState.AccelerateOrSetProperSpeed
                                         ? _accelerationSounds
                                         : _decelerationSounds;
                var numSounds  = audioClips.Length;
                var speedIndex = (int) Mathf.Round((_currentSpeed - MIN_SPEED) / _speedDiv);
        ...
        private float GetCurrentSmokeEmission() =>
                Mathf.Lerp(_standardRateOverTimeMultiplier, _standardRateOverTimeMultiplier * SMOKE_SPEED_MULTIPLIER,
                        _currentSpeed / (MAX_SPEED - MIN_SPEED));
