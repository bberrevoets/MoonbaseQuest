...
/// <summary>
///     When this component is enabled, the player will be able to aim and trigger teleport behavior using Oculus Touch
///     controllers.
/// </summary>
public class TeleportInputHandlerTouch : TeleportInputHandlerHMD
{
    /// <summary>
    ///     These buttons are used for selecting which capacitive button is used when
    ...
    public enum InputModes
    {
        ...
        /// <summary>
        ///     A thumbstick in any direction is used for starting the aiming, and releasing the thumbstick will trigger the
        ///     teleport.
        /// </summary>
        ThumbstickTeleport,

        /// <summary>
        ///     A thumbstick forward or back is used for starting the aiming, and releasing the thumbstick will trigger the
        ///     teleport.
        /// </summary>
    ...
    /// <summary>
    ///     Based on the input mode, controller state, and current intention of the teleport controller, return the apparent
    ...
    public override LocomotionTeleport.TeleportIntentions GetIntention()
    {
        ...
        if (InputMode == InputModes.ThumbstickTeleport || InputMode == InputModes.ThumbstickTeleportForwardBackOnly)
        {
            ...
            var leftStick      = OVRInput.Get(OVRInput.RawAxis2D.LThumbstick);
            var rightStick     = OVRInput.Get(OVRInput.RawAxis2D.RThumbstick);
            var leftMag        = 0.0f;
            var rightMag       = 0.0f;
            var bestMag        = 0.0f;
            var bestController = OVRInput.Controller.Touch;
            var leftTouched    = OVRInput.Get(OVRInput.RawTouch.LThumbstick);
            var rightTouched   = OVRInput.Get(OVRInput.RawTouch.RThumbstick);
            ...
            var touching = bestMag > ThumbstickTeleportThreshold
    ...
    {
        var sourceController = AimingController;
        if (sourceController == OVRInput.Controller.Touch)
        {
            sourceController = InitiatingController;
        }

        var t = (sourceController == OVRInput.Controller.LTouch) ? LeftHand : RightHand;
