...
{
    /// <summary>
    ///     The aiming system uses a common function for testing collision with the world, which can be configured to use
    ///     different
    ///     shapes for testing.
    /// </summary>
    ...
    /// <summary>
    ///     Radius of the sphere or capsule used for collision testing when aiming to possible teleport destinations. Ignored
    ///     if UseCharacterCollisionData is true.
    /// </summary>
    ...
    /// <summary>
    ///     Height of the capsule used for collision testing when aiming to possible teleport destinations. Ignored if
    ///     UseCharacterCollisionData is true.
    /// </summary>
    ...
    /// <summary>
    ///     AimCollisionTest is used by many of the aim handlers to standardize the testing of aiming beams. By choosing
    ///     between the increasingly restrictive
    ///     point, sphere and capsule tests, the aiming system can limit targeting to routes which are not physically blocked.
    ///     For example, a sphere test
    ...
    protected void CreateNewTeleportDestination()
    {
        ...
        var td = Instantiate(TeleportDestinationPrefab);
    ...
    public void DoTeleport()
    {
        ...
        var destPosition = destTransform.position;
        destPosition.y += character.height * 0.5f;
        var destRotation = _teleportDestination.LandingRotation; // destTransform.rotation;
        #if false
    ...
    /// </summary>
    /// <returns></returns>
    public Vector3 GetCharacterPosition() => LocomotionController.CharacterController.transform.position;
    ...
    public Quaternion GetHeadRotationY()
    {
        ...
        #endif
        var euler = headRotation.eulerAngles;
        euler.x      = 0;
        euler.z      = 0;
    ...
    ///     Movement may not be desired at all states, for instance during aiming it may be preferred to prevent movement to
    ///     allow a
    ...
    ///     Rotation may not be desired at all states, for instance during aiming it may be preferred to prevent rotation (snap
    ///     turn or linear)
    ...
    /// <summary>
    ///     This event is raised when the teleport destination is in the process of being updated. It is used by the active
    ///     TeleportDestination
    ///     to update it's visual state, position and orientation indicator to match the results of the teleport aim and
    ///     targeting system.
    /// </summary>
    public event Action<bool, Vector3?, Quaternion?, Quaternion?> UpdateTeleportDestination;

    /// <summary>
    ///     When the active aim and orientation handler finishes preparing the data for the teleport destination, this method
    ///     will be called
    ///     in order to raise the UpdateTeleportDestination event, which will in turn give any active teleport destination
    ///     objects an opportunity
    ...
    /// <summary>
    ///     The state machine will not exit the Teleporting state while IsTransitioning is true. This is how the
    ///     BlinkTransition and WarpTransition
    ...
    ///     * Parabolic: player capsule swept along a series of line segments approximating a parabolic curve until it hits
    ///     terrain or valid target.
    ///     Target Handlers:
    ///     * NavMesh = Destination only valid if it lies within the nav mesh.
    ///     * Node = Destination valid if within radius of a teleport node. Target is invalidated when aim leaves the node
    ///     radius.
