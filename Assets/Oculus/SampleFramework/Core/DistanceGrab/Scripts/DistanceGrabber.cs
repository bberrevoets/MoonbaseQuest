...
namespace OculusSampleFramework
{
    ...
    public class DistanceGrabber : OVRGrabber
    {
        ...
        [SerializeField] private float m_noSnapThreshhold = 0.05f;

        [SerializeField] private bool m_useSpherecast;

        // Public to allow changing in demo.
        [SerializeField]
        public bool m_preventGrabThroughWalls;

        [SerializeField] private float m_objectPullVelocity = 10.0f;

        // Objects can be distance grabbed up to this distance from the hand.
        [SerializeField] private float m_maxGrabDistance;

        // Only allow grabbing objects in this layer.
        // NOTE: you can use the value -1 to attempt to grab everything.
        [SerializeField] private int m_grabObjectsInLayer = 0;

        [SerializeField] private int m_obstructionLayer = 0;
        ...
        protected override void Start()
        {
            ...
            var sc = m_player.GetComponentInChildren<Collider>();
            if (sc != null)
            {
            ...
            var grabbers = FindObjectsOfType<DistanceGrabber>();
        ...
        {
            var closestGrabbable         = m_target;
            var closestGrabbableCollider = m_targetCollider;
            ...
            if (closestGrabbable != null)
            {
                ...
                var closestPointOnBounds = closestGrabbableCollider.ClosestPointOnBounds(m_gripTransform.position);
                if (!m_grabbedObj.snapPosition && !m_grabbedObj.snapOrientation && m_noSnapThreshhold > 0.0f && (closestPointOnBounds - m_gripTransform.position).magnitude < m_noSnapThreshhold)
                {
                    ...
                    var relOri = Quaternion.Inverse(transform.rotation) * m_grabbedObj.transform.rotation;
        ...
        protected override void MoveGrabbedObject(Vector3 pos, Quaternion rot, bool forceTeleport = false)
        {
            ...
            {
                var travel = m_objectPullVelocity * Time.deltaTime;
                var dir    = grabbablePosition - m_grabbedObj.transform.position;
        ...
        protected bool FindTarget(out DistanceGrabbable dgOut, out Collider collOut)
        {
            ...
            foreach (var cg in m_grabCandidates.Keys)
            {
                var grabbable = cg as DistanceGrabbable;
                var canGrab   = grabbable != null && grabbable.InRange && !(grabbable.isGrabbed && !grabbable.allowOffhandGrab);
                ...
                for (var j = 0; j < grabbable.grabPoints.Length; ++j)
                {
                    ...
                    if (grabbableMagSq < closestMagSq)
                    {
                        ...
                        if (m_preventGrabThroughWalls)
                        {
                            ...
                            {
                                var distToObject = (grabbableCollider.ClosestPointOnBounds(m_gripTransform.position) - m_gripTransform.position).magnitude;
        ...
        protected bool FindTargetWithSpherecast(out DistanceGrabbable dgOut, out Collider collOut)
        {
            dgOut   = null;
            collOut = null;
            var        ray = new Ray(m_gripTransform.position, m_gripTransform.forward);
            RaycastHit hitInfo;
            ...
            // essential.
            var layer = (m_grabObjectsInLayer == -1) ? ~0 : 1 << m_grabObjectsInLayer;
